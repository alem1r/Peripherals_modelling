...
void HELPER(trace_insn)(CPUArchState *env, uint64_t pc)
{

    bool is_thumb = env->thumb;
    if(is_thumb){

        uint16_t instruction = cpu_lduw_code(env, pc);
        uint16_t opcode_group = (instruction >> 10) & 0x3F;
        uint8_t sub_opcode = (instruction >> 6) & 0xF;
       
        uint16_t opc_group = opcode_group;
        uint8_t opc_sub = sub_opcode;
       
        //capture xor operation
        if (opc_group==0x10 && opc_sub==0x1){

            opc_logical = opc_group;
            opc_xor_logical = opc_sub;

            //fprintf(file_pointer,"THUMB PROGRAM COUNTER 0x%x OP_GROUP 0x%x OPC_SUB 0x%x\n", pc,opc_group,opc_sub);
            //fflush(file_pointer);
        ...
        uint32_t opcode = cpu_lduw_code(env, pc);
        // Decode ARM opcode
        uint32_t cond = (opcode >> 28) & 0xF; // Condition field
        uint32_t opcode_type = (opcode >> 21) & 0xF; // Opcode field

        
        fprintf(file_pointer,"PROGRAM COUNTER 0x%x OP 0x%x Cond 0x%x OP_type 0x%x\n", pc,opcode,cond,opcode_type);
        fflush(file_pointer);
        
    }

    //fprintf(file_pointer,"PROGRAM COUNTER 0x%x\n", pc);
    //fflush(file_pointer);
    
    
}

static void *trace_load_guest_internal(CPUArchState *env, uint64_t pc,
                                     uint64_t value, target_ulong addr,
                                     uint64_t load_length, uint8_t result_length,
                                     target_ulong mmu_idx)
{
    //fprintf(file_pointer,"Load From: 0x%x, PC 0x%x, Value 0x%x\n", addr, pc, value);
    //fflush(file_pointer);

   
}

void HELPER(trace_load_guest_i32)(CPUArchState *env, uint64_t pc, 
                                 uint32_t value,
                                 target_ulong addr,
                                 uint64_t length, target_ulong mmu_idx)
{
    trace_load_guest_internal(env, pc, value, addr, length, 4, mmu_idx);
}

void HELPER(trace_load_guest_i64)(CPUArchState *env, uint64_t pc,
                                 uint64_t value,
                                 target_ulong addr,
                                 uint64_t length, target_ulong mmu_idx)
{
    trace_load_guest_internal(env, pc, value, addr, length, 8, mmu_idx);
}

static void trace_store_guest_internal(CPUArchState *env, uint64_t pc,
                                     uint64_t value,
                                     target_ulong addr,
                                     uint64_t length, target_ulong mmu_idx)
{
    //fprintf(file_pointer,"Store From: 0x%x, PC 0x%x, Value 0x%x\n", addr, pc, value);
    //fflush(file_pointer);

   
}

void HELPER(trace_store_guest_i32)(CPUArchState *env, uint64_t pc, 
                                 uint32_t value,
                                 target_ulong addr,
                                 uint64_t length, target_ulong mmu_idx)
{
    return trace_store_guest_internal(
        env, pc, value, addr, length, mmu_idx);
}

void HELPER(trace_store_guest_i64)(CPUArchState *env, uint64_t pc,
                                 uint64_t value,
                                 target_ulong addr,
                                 uint64_t length, target_ulong mmu_idx)
{
...
static void *trace_load_host_internal(CPUArchState *env, uint64_t pc, uint64_t value, void *addr, uint64_t offset,
...
void HELPER(trace_load_host_i32)(CPUArchState *env, uint64_t pc, uint32_t value,
                                void *addr,
                                uint64_t offset, uint64_t length)
{
    trace_load_host_internal(env, pc, value, addr, offset, length, 4);
}

void HELPER(trace_load_host_i64)(CPUArchState *env, uint64_t pc, uint64_t value,
                                void *addr,
                                uint64_t offset, uint64_t length)
{
    trace_load_host_internal(env, pc, value, addr, offset, length, 8);
}

void HELPER(trace_store_host_i32)(CPUArchState *env, uint64_t pc, uint32_t value,
                                void *addr,
                                uint64_t offset, uint64_t length)
{
    //fprintf(file_pointer,"Store From: 0x%x, PC 0x%x, Value 0x%x\n", addr, pc, value);
...
void HELPER(trace_store_host_i64)(CPUArchState *env, uint64_t pc, uint64_t value,
                                void *addr,
                                uint64_t offset, uint64_t length)
{
    //fprintf(file_pointer,"Store From: 0x%x, PC 0x%x, Value 0x%x\n", addr, pc, value);
