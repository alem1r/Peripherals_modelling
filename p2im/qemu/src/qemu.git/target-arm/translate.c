...
volatile target_ulong cur_pc = 0;
#endif
#endif
...
{
	/*
	FILE *fp = fopen("/tmp/disas_arm_insn.log", "a+");
	fprintf(fp, "pid = %d, pc = %08x, tb-pc = %08x\n", getpid(), s->pc, s->tb->pc);
	fclose(fp);
	*/
...
{
	/*
	FILE *fp = fopen("/tmp/disas_thumb2_insn.log", "a+");
	fprintf(fp, "disas_thumb2_insn = %08x\n", insn_hw1);
	fclose(fp);
	*/
                                                  ...
                                                  bool search_pc)
{
    ...
    do {
		...
		// Chunlin: record the current pc
#ifdef __OPEN_MULTI_STATUS__
		cur_pc = dc->pc;
#endif
//#define __OPEN_DETAIL_LOG
#ifdef __OPEN_DETAIL_LOG
		char path_log[50];
		sprintf(path_log, "/tmp/disas_arm_insn_%d", getpid());
        ...
        if (dc->thumb) {
            insn_pc = dc->pc;
            TCGv_i64 pc = tcg_const_i64(insn_pc);
            gen_helper_trace_insn(tcg_ctx.tcg_env, pc);
        ...
        num_insns ++;
             ...
             num_insns < max_insns);
	//Chunlin: Here, we catch the address where condition jump happen.
	/*
	if(dc->is_jmp) { 
		printf("\ndc->is_jmp = %d,\tpc_address = %p\n", dc->is_jmp, dc->pc);
	}
	*/
...
*/
    ...
    pm_rand_sz = ((end - p_prev) > len_seeds) ? len_seeds : (end - p_prev);
...
target_ulong helper_aflCall(CPUArchState *env, target_ulong code, target_ulong a0, target_ulong a1) {
	...
	printf("helper_aflCall-code = %x, a0 = %x, a1 = %x\n", code, a0, a1);
    switch(code) {
