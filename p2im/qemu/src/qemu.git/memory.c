...
static uint64_t last_read_val = 0;        // Last value read from a specific address
static hwaddr last_read_addr = 0;         // Address of the last read operation
static bool last_access_was_read = false; // Flag indicating if the last access was a read
static const uint64_t first_xor_value = 0x2000;    // XOR mask for detection (example mask)
static const uint64_t second_xor_value = 0x4000;    // XOR mask for detection (example mask)
static const uint64_t third_xor_value = 0x8000;    // XOR mask for detection (example mask)
//ending patch global variables
...
//#endif
    ...
    if(pm_ena && (addr >= 0x40000000 && addr < 0x60000000)) 
    {
        ...
        if(strcmp(cs_g->mcu_name, "STM32F103RB") == 0)
		{
            if(addr32==0x4001100c) {
                reg->type = DR;
				SR_cat_by_fixup = 0;
            }
            //patch USB on F103
            else if(addr32==0x40005c44) {
                reg->type = SR;
                reg->sr_locked = true;
            }
        ...
        if(strcmp(cs_g->mcu_name, "STM32F103RB") == 0) {
            ...
            else if(addr32 == 0x40000024 || addr32 == 0x48000010 ||
		...
		*/
        //adding patch
        last_read_val = ret_val;      // `ret_val` is the value read from the address
        last_read_addr = addr;         // Address being read from
        last_access_was_read = true;   // Mark this access as a read
        //ending patch
...
/*
    //adding patch
        if(strcmp(cs_g->mcu_name, "STM32F103RB") == 0) {    
            ...
            if ( (last_access_was_read && addr == last_read_addr) && addr != 0x40021018 && addr != 0x4001380c ) {

                uint32_t diff = last_read_val ^ val;

            // Check if the difference corresponds to a valid XOR mask
    if (diff == 0x2000 || diff == 0x4000 || diff == 0x8000) {
        // Confirm it's XOR: all differing bits must match the mask
        if ((val & diff) == diff) {
            reg->type = DR;
            SR_cat_by_fixup = 0;

            fprintf(file_pointer, "ADDR 0x%x Value BEFORE 0x%x Value AFTER 0x%x (XOR with 0x%x)\n", 
                    addr, last_read_val, val, diff);
            fflush(file_pointer);
        }
           }
            // Update tracking variables for the next write
            last_access_was_read = false;
            //end patch
        }
...
/*
        ...
        if(strcmp(cs_g->mcu_name, "STM32F103RB") == 0) {    
            ...
            if ( (last_access_was_read && addr == last_read_addr)  && (addr >= 0x40011000 && addr < 0x40011010) ) {

               

            // Check if current write is a XOR operation with the last write value
                if  ( ( val == (last_read_val ^ first_xor_value) ) && (opc_logical==0x10 && opc_xor_logical==0x1) ||
                      ( val == (last_read_val ^ second_xor_value) ) && (opc_logical==0x10 && opc_xor_logical==0x1) ||
                      ( val == (last_read_val ^ third_xor_value) ) && (opc_logical==0x10 && opc_xor_logical==0x1 ) )
            {
                    
                    reg->type = DR;
				    SR_cat_by_fixup = 0;
                    //fprintf(file_pointer,"ADDR 0x%x Value BEFORE 0x%x Value AFTER 0x%x OPC_GROUP 0x%x OPC_SUB 0x%x\n",addr,last_read_val,val,opc_group,opc_sub);
                    //fflush(file_pointer);
                    fprintf(file_pointer,"ADDR 0x%x Value BEFORE 0x%x Value AFTER 0x%x\n",addr,last_read_val,val);
                    fflush(file_pointer);
           ...
           //adding patch
        if(strcmp(cs_g->mcu_name, "STM32F429ZI") == 0) {    

            if ( (last_access_was_read && addr == last_read_addr)  ) {

               

            // Check if current write is a XOR operation with the last write value
                if  ( ( val == (last_read_val ^ 0x1000) ) && (opc_logical==0x10 && opc_xor_logical==0x1)  )
            {
