...
static u32 bbl_cnt;                   /* current number of basic block    */
...
EXP_ST u8* bbl_bits;                  /* SHM with basic block bitmap      */

EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           virgin_tmout[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
           virgin_crash[MAP_SIZE],    /* Bits we haven't seen in crashes  */
           bitmap_bbl[MAP_SIZE];      /* Bitmap of basic block            */

static u8  var_bytes[MAP_SIZE];       /* Bytes that appear to be variable */

static s32 shm_id;                    /* ID of the SHM region             */
static s32 shm_id2;                   /* ID of the SHM region - bbl-cov   */
...
EXP_ST void write_bitmap_bbl(void) {
  ...
  fname = alloc_printf("%s/bitmap_bbl", out_dir);
  fd = open(fname, O_WRONLY | O_CREAT | O_TRUNC, 0600);
  if (fd < 0) PFATAL("Unable to open '%s'", fname);

  ck_write(fd, bitmap_bbl, MAP_SIZE, fname);
...
EXP_ST void write_bblcnt_log(int cnt1) {
  ...
  fname = alloc_printf("%s/bbl_log", out_dir);
  fd = open(fname, O_CREAT | O_APPEND | O_RDWR, 0600);
  if (fd < 0) PFATAL("Unable to open '%s'", fname);

  char log_buf[40];
  sprintf(log_buf, "cnt=%d time=%s\n", cnt1, get_date_time());
  ck_write(fd, log_buf, strlen(log_buf), fname);
...
static inline u32 update_bitmap_bbl() {
  ...
  u64* current2 = (u64*)bbl_bits;
  u64* virgin2  = (u64*)bitmap_bbl;

  u32  i = (MAP_SIZE >> 3);

#else

  u32* current2 = (u32*)bbl_bits;
  u32* virgin2  = (u32*)bitmap_bbl;
  ...
  // in order to evaluation, we just write the log to file
  if (cnt > bbl_cnt) {
  	write_bblcnt_log(cnt);
  }
  return cnt;
}
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) memset(bitmap_bbl, 0, MAP_SIZE);
  ...
  if (!dumb_mode) setenv(SHM_ENV_VAR2, shm_str2, 1);

  ck_free(shm_str);
  ck_free(shm_str2);

  trace_bits = shmat(shm_id, NULL, 0);
  bbl_bits = shmat(shm_id2, NULL, 0);
  
  if (trace_bits == (void *)-1) PFATAL("shmat() failed");
  if (bbl_bits == (void *)-1) PFATAL("shmat() failed");
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
	...
	//int fd = open("/tmp/initforkserver.log",O_RDWR|O_CREAT, 0644);
    dup2(dev_null_fd, 1);
    dup2(dev_null_fd, 2);
    //dup2(fd, 1);
    //dup2(fd, 2);
...
#ifdef NEED_LOG
static volatile int times_num = 0;
#endif
 ...
 RERUN_AFTER_ME:
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (!child_pid) {
      ...
      setsid();
#ifdef NEED_LOG
	  //char name_out[50], name_err[50];
	  //sprintf(name_out, "//tmp//afl-out%d.log", times_num);
	  //sprintf(name_err, "//tmp//afl-err%d.log", times_num);
	  //int fd_out = open(name_out, O_RDWR|O_CREAT, 0644);
	  //int fd_err = open(name_err, O_RDWR|O_CREAT, 0644);
	  int fd_out = open("/tmp/afl-out.log", O_RDWR|O_CREAT, 0644);
	  int fd_err = open("/tmp/afl-err.log", O_RDWR|O_CREAT, 0644);
      dup2(fd_out, 1);
      dup2(fd_err, 2);
	  close(fd_out);
	  close(fd_err);
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  //adding code here
  // Define a buffer filled with 0xFF instead of using the 'mem' buffer passed to the function
  //unsigned char fixed_byte = 0xAB;
  ...
  //uncomment here for original code
  ck_write(fd, mem, len, out_file);
  // Write the fixed buffer to the file instead of 'mem'
  //ck_write(fd, &fixed_byte, 1, out_file);
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  fprintf(f, "start_time        : %llu\n"
             ...
             queued_paths, update_bitmap_bbl(), queued_favored, 
			 queued_discovered, queued_imported,
...
#define SP20    SP10 SP10
  ...
  SAYF(bV bSTOP "        trim : " cRST "%-37s " bSTG bV bSTOP
       "   bbl cnt : " cRST "%-10d " bSTG bV "\n", tmp, bbl_cnt);

  SAYF(bLB bH30 bH20 bH2 bH bHT bH20 bH2 bH2 bRB "\n" RESET_G1);
  ...
  if (cpu_core_count) {
    ...
    if (cpu_aff >= 0) {
      ...
      SAYF(SP20 SP20 SP20 SP5 cGRA "[cpu%03u:%s%3u%%" cGRA "]\r" cRST, 
...
static char** get_qemu_argv(u8* own_loc, char** argv, int argc) {
  ...
  if (qemu_mode == 1) {
    ...
    if (no_forkserver) {
	  ...
	  new_argv[argc+2] = "-run-from-afl";
      new_argv[argc+3] = NULL; // redudant, since ck_alloc zero'd new_argv
    }
